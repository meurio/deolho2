...
RSpec.feature "Remove an adopter from a project" do
  ...
  scenario "when I'm an admin" do
    ...
    expect(page).to have_css("li.user a[data-method='delete']")
    expect(page).to have_css("li#adoption-#{@adoption.id}")

    click_link "remove-adoption-#{@adoption.id}"

    expect(current_path).to eq(project_path(@project))
    expect(page).to_not have_css("li#adoption-#{@adoption.id}")
  end
